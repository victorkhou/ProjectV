...
- **Refactor delivery states to a `StrEnum`** — The delivery FSM uses raw strings (`"idle"`, `"picking_up"`, `"delivering"`, `"returning"`) in ~20 places across `agent_scripts.py` and `agent_system.py`. Extract a `DeliveryState(StrEnum)` in `world/constants.py` so typos are caught at import time and IDE autocomplete works. `StrEnum` serializes to plain strings so Evennia attribute persistence is unaffected.
- **Refactor activity status strings to constants or an enum** — `"Idle"`, `"Blocked — waiting"`, `"Patrol blocked — retrying"`, etc. appear as raw strings in `npcs.py`, `agent_scripts.py`, and `agent_system.py`. At minimum, extract `ACTIVITY_IDLE = "Idle"` to `world/constants.py`; consider a full enum if the set of statuses stabilizes.
- **Integrate equipment speed modifier into `NPC.advance_movement`** — Property 14 tests a standalone `compute_effective_delay` function. The actual `advance_movement` method doesn't yet query `EquipmentHandler` for a `"move_speed"` stat modifier. Wire `effective_delay = max(1, base_delay - modifier)` into the delay gating logic.
