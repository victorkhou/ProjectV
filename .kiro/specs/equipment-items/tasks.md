...
- [ ] 1. Item data model, constants, and schema validation
  - [ ] 1.1 Extend `ItemDef` in `world/definitions.py`
  ...
  - [ ] 1.2 Add equipment constants to `world/constants.py`
  ...
  - [ ] 1.3 Extend the item populator in `world/data_registry.py`
  ...
  - [ ] 1.4 Add `resource_weights` to `BalanceConfig` (`world/definitions.py`) + balance plumbing
  ...
  - [ ] 1.5 Extend `SchemaValidator.validate_items` in `world/schema_validator.py`
  ...
  - [ ] 1.6 Extend cross-validation in `world/schema_validator.py`
  ...
  - [ ] 1.7 Expose the new fields on `GameItem` (`typeclasses/objects.py`)
    ...
    - _Requirements: 14.4_
  - [ ] 1.8 Property test for item + balance schema validation
...
- [ ] 2. Supply_Bag storage on `EquipmentHandler`
  - [ ] 2.1 Add Supply_Bag helpers to `world/systems/equipment_handler.py`
  ...
  - [ ] 2.2 Unit + property tests for the Supply_Bag
...
- [ ] 3. `EquipmentSystem` use-case: equip / unequip / use / throw / reload
  - [ ] 3.1 Add `equip(player, item)` with rank gating in `world/systems/equipment_system.py`
  ...
  - [ ] 3.4 Add `throw(player, item_key, tx, ty)` for throwables
  ...
  - [ ] 3.5 Add `reload(player)` for the equipped ranged weapon
  ...
  - [ ] 3.6 Add `add_supply_drop(player, item_key, count)` (weight- and stack-aware pickup)
  ...
  - [ ] 3.7 Inject the area-damage applier at the composition root
  ...
  - [ ] 3.8 Unit tests for the use-case
...
- [ ] 4. Combat engine — the two additive touches (`world/systems/combat_engine.py`)
  - [ ] 4.1 Add `damage_bonus` aggregation
  ...
  - [ ] 4.2 Melee gating + magazine draw in attack-queue validation
...
- [ ] 5. Presenter — new notification kinds (`world/presenters/notification_presenter.py`)
  - [ ] 5.1 Add formatters to `_FORMATTERS` (all 11 kinds)
  ...
  - [ ] 5.2 End-to-end presenter tests
...
- [ ] 7. Player-facing commands (`commands/game_commands.py`)
  - [ ] 7.1 Re-route `equip`/`unequip` through `EquipmentSystem`
    - Use `require_system("equipment_system")`; preserve message shape
    - _Requirements: 12.1, 12.2_
  - [ ] 7.2 Paperdoll `equipment` command
  ...
  - [ ] 7.3 `inventory`/`score` Supplies + carried-weight section
  ...
  - [ ] 7.4 New `use`, `throw`, and `reload` commands
  ...
  - [ ] 7.5 New `deposit`/`withdraw` commands
  ...
  - [ ] 7.6 `score` includes aggregated equipment totals
    - _Requirements: 12.10_
  - [ ] 7.7 Command unit tests
...
- [ ] 8. Content and production
  - [ ] 8.1 Migrate existing items in `data/definitions/items.yaml`
    ...
    - _Requirements: 13.2_
  - [ ] 8.2 Seed the starter set + weights
  ...
  - [ ] 8.3 Extend the production map
    - `AR → weapons+ammo`, `AA → armor`, `MB → consumables`, `LB → futuristic gear+throwables`
    - _Requirements: 13.3_
  - [ ] 8.4 Route production by category in `EquipmentSystem.process_production`
  ...
  - [ ] 8.5 Content-load + reload tests
    - Migrated + seeded content loads clean; `@reloaddata` swaps atomically
    - _Requirements: 13.5, 13.6_

- [ ] 9. Weight carry cap, Vault/HQ storage, and inflow choke point (D7/D8/D9)
  - [ ] 9.1 `carried_weight` / `carry_limit` on `EquipmentSystem`
  ...
  - [ ] 9.2 `add_resource_capped(holder, resource, amount)` inflow choke point
  ...
  - [ ] 9.3 Route the holder-pool inflow paths through the choke point
    ...
    - _Requirements: 16.7_
  - [ ] 9.4 Real Storage_Building pool
    ...
    - _Requirements: 16.1_
  - [ ] 9.5 `deposit`/`withdraw` use-case methods
  ...
  - [ ] 9.6 Redirect harvester delivery deposit to the Storage_Building
...
- [ ] 10. Stale-test reconciliation
  - [ ] 10.1 Update fixtures assuming the old slot strings / `ItemDef` shape / count-based carry
    - Adjust any test constructing items with `gadget`/`consumable` slots, the old 7-field `ItemDef`, or `BASE_CARRY_CAPACITY`
    - _Requirements: 14.3, 14.4_
  - [ ] 10.2 Confirm zero-equipment identity + spend-pool preservation
...
- [ ] 11. Final integration
  - [ ] 11.1 Wire `EquipmentSystem` collaborators + any new events in `server/conf/game_init.py`
  ...
  - [ ] 11.2 Full-suite green + layering guard
