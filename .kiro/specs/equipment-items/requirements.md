...
- **D6 — `max_hp` from equipment: intended but deferred (resolved from Open Decision O2).** Equipped gear *may* raise a character's maximum HP via a `max_hp` stat, and this is a desired capability — but it is **low priority and out of scope for this feature**. For now `max_hp` remains a declared, load-validated numeric stat with no HP effect (Requirement 6.4). Wiring `Σ max_hp` into `hp_max` (and clamping current HP on unequip) is a documented, ready follow-up (design "Resolved decisions & deferred follow-up" / task 6.4), not part of the initial delivery.
...
4. THE Schema_Validator SHALL accept `max_hp` and `accuracy` as valid numeric stat keys on Gear but THE system SHALL apply no combat or HP effect from them in this feature (reserved for a later pass; see decision D6).
