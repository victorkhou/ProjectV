...
checked — the feature is implemented and covered by unit tests plus the
guarded live-boot smoke test. One design correction surfaced during
implementation and is worth recording: the real-player guard behind `are_allied`
must NOT gate on `has_account` (Evennia defines it as the live *session count*,
so an offline real player would fail it and an offline ally would wrongly be
treated as hostile / un-invitable). It instead identifies a player structurally
(a `combat_xp`-carrying character that is neither a Sentinel nor an `npc_type`
unit). The live-boot smoke test caught this — the stubbed suite could not.
...
- [x] 1. Data model + persistence foundation
  - [x] 1.1 Add `player_alliance=None`, `alliance_rank=None`,
  ...
  - [x] 1.2 Add `ALLIANCE_RANKS`, `ALLIANCE_RANK_ORDER`,
    `ALLIANCE_PERK_CATEGORIES`, and `ALLIANCE_NAME_DENYLIST` to
    `world/constants.py`.
    - _Requirements: 5.1, 9.7, 19.2_
  - [x] 1.3 Implement `AllianceRegistry(DefaultScript)` (persistent;
  ...
  - [x] 1.4 Roster enumeration helper using `search_object_attribute(key=
...
- [x] 2. The single ally predicate (`world/utils.py`)
  - [x] 2.1 Implement `are_allied(a, b)` alongside `is_owner`: two distinct REAL
...
- [x] 3. AllianceSystem — membership authority (single writer)
  - [x] 3.1 `AllianceSystem(BaseSystem)`; `found` with level gate
  ...
  - [x] 3.2 `invite` / `accept` / `decline`: pending-invite bookkeeping with
  ...
  - [x] 3.3 `apply`/`request` (INBOUND `pending_requests`) + `accept_request`
  ...
  - [x] 3.4 `leave` / `kick` / `disband` / `transfer`: strictly-lower-rank kick
  ...
  - [x] 3.5 `promote` / `demote` and the role-permission matrix (Leader-only vs
  ...
  - [x] 3.6 One-alliance-per-player + member-cap invariants enforced before every
  ...
  - [x] 3.7 Succession + chardelete + claim: `on_character_deleted` routes deletion
  ...
  - [x] 3.8 `rename` / `retag`: re-run the founding validators (R2.4, R19.1–R19.3),
  ...
  - [x] 3.9 All mutations use read-modify-reassign for treasury/roster/
    active_perks/pending_invites/pending_requests/withdraw_window; value-based
    reads throughout.
    - _Requirements: 14.3, 14.4_

- [x] 4. Shared treasury
  - [x] 4.1 `deposit`: RE-READ treasury before write-back (C9); add to `treasury`
  ...
  - [x] 4.2 `withdraw`: Officer+ only; enforce the per-window Withdrawal_Cap
  ...
  - [x] 4.3 Even-split treasury on disband (and no-member succession): credit each
...
- [x] 5. Alliance level derivation
  - [x] 5.1 `compute_alliance_level` from the SUM of member `get_player_level`
...
- [x] 6. Perk tiers — unlock + activation
  - [x] 6.1 Author `data/definitions/alliance_perks.yaml` (the v1 catalog: one perk
  ...
  - [x] 6.2 `available_perks` / `activate_perk`: apply BOTH gates (level tier
  ...
  - [x] 6.3 Activated perk stays active if level later drops (GRANDFATHERED, no
    retro-revoke); settled intended behavior.
    - _Requirements: 9.6, 16.3_

- [x] 7. Concrete perk effects (membership-derived hooks)
  - [x] 7.1 Shared vision: add a shared `AllianceSystem.shared_visible_tiles`
  ...
  - [x] 7.2 Fog `update_discovery`: do NOT flag an allied member's building as an
  ...
  - [x] 7.3 Shared regen: register a `(entity)->float` MULTIPLIER provider via
    `RegenSystem.add_modifier_provider` returning the perk multiplier for members,
    `1.0` otherwise.
    - _Requirements: 10.2, 10.5_
  - [x] 7.4 Harvest perk: scale member active-presence yield at
    ...
    scope in v1.
    - _Requirements: 10.3_
  - [x] 7.5 Combat perk: add the FLAT `damage_bonus` term at `_get_attacker_bonus`
  ...
  - [x] 7.6 Assert perks never touch base ownership (`owner_has_active_hq` /
    `active_hq_owner_ids` unchanged; no ally-territory build / shared structures).
    - _Requirements: 10.6, 16.1_

- [x] 8. Friendly fire — hybrid rule (combat seams)
  - [x] 8.1 `process_turrets` (`combat_engine.py`): add `or are_allied(player,
    owner)` to the target-loop continue-guard so turrets skip allies.
    - _Requirements: 11.1_
  - [x] 8.2 `guard_combat_system._acquire_target`: add `or are_allied(player,
    owner)` to the `is_owner` continue-guard so guards/agents skip allies.
    - _Requirements: 11.2_
  - [x] 8.3 Extend the XP guards against Owning_Players: in `_handle_player_defeat`
  ...
  - [x] 8.4 Increment the SPLIT decaying tallies on the Owning_Player on the
...
- [x] 9. Leaderboard + member board
  - [x] 9.1 `_decayed_kills` helper (lazy decay of `scored_kills_pvp` /
  ...
  - [x] 9.2 `leaderboard` (desc score, asc-id tiebreak, deterministic, truncated to
    `alliance_leaderboard_top_n`); `member_board` reusing `RankSystem.get_status`
    rows.
    - _Requirements: 13.2, 13.3_
  - [x] 9.3 Add the split score weight + decay scalars
...
- [x] 10. Commands + events + wiring
  - [x] 10.1 `CmdAlliance(GameSubcommandRouter)` in
  ...
  - [x] 10.2 Implement the verb-aware lobby gate: `MUTATING_LOBBY_VERBS`
  ...
  - [x] 10.3 Combat gate the side-changing verbs (`leave`, `transfer`, `disband`,
    `kick`) via the existing `player_in_combat` check; leave `deposit`/`withdraw`
    NOT gated (documented).
    - _Requirements: 22.1, 22.2_
  - [x] 10.4 Add all alliance events to `world/event_bus.py` (`ALLIANCE_CREATED`,
  ...
  - [x] 10.5 Change notifications (C12): on join/leave/kick/promote/demote/
    ...
    published events.
    - _Requirements: 15.9_
  - [x] 10.6 Optionally subscribe to `LEVEL_CHANGED` / `PLAYER_ELIMINATED` ONLY to
    invalidate the alliance-level memo (never mutate a pointer); omit the
    subscription if no memo is kept.
    - _Requirements: 15.5, 8.2_
  - [x] 10.7 Construct `AllianceSystem`, ensure `AllianceRegistry` exists
  ...
  - [x] 10.8 Add the genuine balance scalars (`alliance_found_min_level`,
  ...
  - [x] 10.9 Alliance chat: Account-level channel keyed by the immutable
  ...
  - [x] 10.10 Route chardelete of any member through `AllianceSystem`
    (`on_character_deleted`) in `lifecycle_commands.py` so a deleted member is an
    implicit leave, never an orphaned pointer.
    - _Requirements: 4.7_
  - [x] 10.11 Invite inbox + throttling + ignore: `invites` verb (inbox listing),
...
- [x] 11. Alliance-tag visibility (R20)
  - [x] 11.1 `AllianceSystem.tag_for(player)` helper; render `[TAG] Name` in the
    `who` and `score` commands and the look/tile summary in
    `commands/game_commands.py` (friend and foe).
    - _Requirements: 20.1, 20.3_
  - [x] 11.2 Add the alliance `tag` (or `None`) to the per-tile player payload in
    `world/coordinate/map_data_provider.py` (currently `{name, linkdead}`).
    - _Requirements: 20.2, 20.3_

- [x] 12. Administrative alliance router (R23 / C14)
  - [x] 12.1 `CmdAdminAlliance(AdminSubcommandRouter)` in
  ...
  - [x] 12.2 Route every admin write verb THROUGH `AllianceSystem` (single-writer
    preserved); `force-disband` reuses the even-split + channel-destroy path.
    - _Requirements: 23.2, 23.4_

- [x] 13. Tests
  - [x] 13.1 `world/tests/test_are_allied.py` — the predicate truth table incl.
  ...
  - [x] 13.2 `world/tests/test_alliance_system.py` — founding (free; name/tag
  ...
  - [x] 13.3 `world/tests/test_alliance_combat_seams.py` — turret/guard skip
  ...
  - [x] 13.4 `world/tests/test_alliance_perks.py` — per-ally vision union
  ...
  - [x] 13.5 `commands/tests/test_alliance_commands.py` — verb routing (all verbs),
  ...
  - [x] 13.6 `commands/tests/test_admin_routers.py` — `CmdAdminAlliance`
  ...
  - [x] 13.7 Tag-visibility tests — `who`/`score`/tile summary render `[TAG] Name`;
    `map_data_provider` payload carries `tag` (or `None`) for friend and foe.
    - _Requirements: 20.1, 20.2, 20.3_
  - [x] 13.8 `tests/test_live_boot_smoke.py` additions — real registry persist
...
- [x] 14. Known constraints (documented, tracked, mostly no-code)
  - [x] 14.1 Assert shallow integration in a test: no alliance code path reads or
    writes `owner_has_active_hq` / `active_hq_owner_ids`.
    - _Requirements: 16.1, 16.5_
  - [x] 14.2 Document the settled behaviors: even-split disband (+ residual
  ...
  - [x] 14.3 Document alliance/class independence (class stays cosmetic) and
